...
Binary plus (_addition_) operator `+{:tact}` is used for adding numbers together. Going beyond the maximum value of an [`Int{:tact}`](/book/integers) will result in an error with [exit code 4](/book/exit-codes#4): `Integer overflow`.
...
Binary minus (_subtraction_) operator `-{:tact}` is used for subtracting numbers from each other. Going beyond the minimum value of an [`Int{:tact}`](/book/integers) will result in an error with [exit code 4](/book/exit-codes#4): `Integer overflow`.
...
Binary double greater than (_bitwise shift right_) operator `<<{:tact}` returns an integer which binary representation is the _left operand_ value shifted by the _right operand_ number of bits to the left. Excess bits shifted off to the left are discarded, and zero bits are shifted in from the right. This is a more effective way to multiply the _left operand_ by $2^n$, where $n$ is equal to the _right operand_. Going beyond the maximum value of an [`Int{:tact}`](/book/integers) will result in an error with [exit code 4](/book/exit-codes#4): `Integer overflow`.
