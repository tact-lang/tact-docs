...
Variable declaration uses the [assignment operator](/book/operators#assignment).
...
The `if...else{:tact}` statement executes a statement if a specified condition evaluates to `true{:tact}`. If the condition is evaluated to `false{:tact}`, another statement in the optional `else{:tact}` clause will be executed.
...
The `repeat{:tact}` loop executes a block of code a specified number of times. Number of repetitions must be a non-negative $32$-bit [`Int{:tact}`](/book/integers). Otherwise an `Integer is of range` exception would be thrown.
...
let x: Int = 10;
do {
    x = x - 1;  // executes this code block at least once
} until (x <= 0);
```