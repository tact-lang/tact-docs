...
struct Fizz { ohmy: map<Int, Int> }
message Buzz { ohmy: map<Int, Int> }

contract Iterated {
    ohmy: map<Int, Int>;

    receive("call to iterate!") {
        let ohmy: map<Int, Int> = emptyMap();
        ohmy.set(0, 42);
        ohmy.set(1, 27);

        self.ohmy = ohmy; // assigning local map to the storage one
        let fizz = Fizz{ ohmy }; // field punning
        let buzz = Buzz{ ohmy }; // field punning

        // Iterating over map in contract storage
        foreach (key, value in self.ohmy) {
            // ...
        }

        // Iterating over map member of a Struct Fizz instance
        foreach (key, value in fizz.ohmy) {
            // ...
        }

        // Iterating over map member of a Message Buzz instance
        foreach (key, value in buzz.ohmy) {
            // ...
        }
    }
}
...
<Callout type="warning" emoji="⚠️">
  ...
  Note, that at the moment `foreach{:tact}` works only with explicitly provided map identifiers or compound expressions, such as field access. That is, returning a map from a function and trying to iterate over its entries won't work:
