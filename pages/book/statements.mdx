...
In Tact, declaring a variable always requires an initial value. However, the type ascription can be omitted and Tact will try to infer it from the initial value:

```tact
let value: Int = 123;           // full declaration
let valueInferred = 123;        // inferred type Int
let valueOptional: Int? = null; // explicit type Int or null
let valueCtx = context();       // inferred type Context, a built-in Struct
