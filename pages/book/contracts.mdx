...
contract Example {
    ...
    init() {
        self.var = 42; // <- referencing the var through self.
    }
}
```
...
Read more about them in their dedicated section: [Functions](/book/functions#getter-functions)

### Receiver functions

Receiver functions in Tact can be one of three kinds:
...
* [`external(){:tact}`](/book/bounced), which don't have a sender and can be sent by anyone in the world.
