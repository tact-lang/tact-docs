...
To make the [`Int{:tact}`][int] value for `mode` field of SendParameters, you just have to combine base modes with optional flags by applying a [`bitwise OR{:tact}`](/book/operators#binary-bitwise-or).
