...
The method `expect(…).toHaveTransaction(){:typescript}` checks that the list of transactions has a transaction matching certaing properties you specify:
...
### Send messages to receivers [#tests-receive]

### Observe the fees [#tests-fees]

### Test transactions with intentional errors [#tests-errors]
...
### Simulate passage of time in tests [#tests-time]
