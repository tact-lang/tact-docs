...
- Allowing assigning a `null` value to a map variable (same as calling `emptyMap()`)
...
- Updated code gen to prefix function names with a `$` to avoid clashing with system functions
...
- `Resumable` trait, allows to resumption of contract operations once it was stopped
...
- Removed some keywords such as `message`, `contract`, `init` to allow the use these names as variable names
