...
For a quick start, it is better to use a `tact-template` that has everything built in.
...
awaiting the contract.send(treasure, { value: toNano(1) }, { $$type: "Deploy", queryId: 0n });
