...
Mutable functions are performing mutation of a value replacing it with an execution result. To perform the mutation function must change the `self` value.
...
Native functions are directly binding to a func function:

> **Note**
> Native functions could be also mutable and extensions of one
...
Getter functions that define getters on smart contracts and can be defined only within a contract or trait.
