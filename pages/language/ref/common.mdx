...
<Callout type="warning" emoji="⚠️">
  ...
  This method throws an error with the [exit code 136](/book/exit-codes#136) if `chain` is invalid or [exit code 137](/book/exit-codes#137) if `chain` points to the masterchain ($-1$) without [masterchain support](/book/masterchain) enabled.
