...
Compiler generates files named `{project}_{contract}.ts` for each contract in your [project](/book/config#projects) that contains ready-to use strongly typed wrappers to work with a contract in any environment: for [testing](/book/debug), [deployments](/book/deploy), etc.
