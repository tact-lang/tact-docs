...
This page lists a handy collection of data structures implemented in Tact for your day-to-day needs.

Most of the data structures here are made using the built-in [`map<k, v>{:tact}`][map] type. For the description and basic usage of maps see the [dedicated page in the Book][map].

## Array

An [array](https://en.wikipedia.org/wiki/Array_(data_structure)) is a continuous block of memory representing a collection of elements of same memory size, each identified by at least one array key or _index_.
